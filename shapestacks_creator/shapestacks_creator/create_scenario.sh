...
create_params()
{
  ...
  height=`python3 -c "import random; print(random.randint(3,${MAX_HEIGHT}))"`
  vcom=`python3 -c "import random; print(random.randint(0,${height}))"`
  shape_num=`python3 -c "import random; print(random.randint(1,3))"`
  shapes=`python3 -c "import numpy as np; [print(name) for name in np.random.choice(['cuboid', 'cylinder', 'sphere'], size=${shape_num}, replace=False)]"`
...
do
        ...
        python3 create_scenario.py ${params}
        i=$(($i+1))
        echo "File $i"
done
